...
# Total jumlah gol dari tim pemenang
echo -e "\nTotal number of goals in all games from winning teams:"
echo "$($PSQL "SELECT SUM(winner_goals) FROM games")"

# Total jumlah gol dari kedua tim
echo -e "\nTotal number of goals in all games from both teams combined:"
echo "$($PSQL "SELECT SUM(winner_goals + opponent_goals) FROM games")"

# Rata-rata jumlah gol dari tim pemenang
echo -e "\nAverage number of goals in all games from the winning teams:"
echo "$($PSQL "SELECT AVG(winner_goals) FROM games")"

# Rata-rata jumlah gol dari tim pemenang (dibulatkan 2 desimal)
echo -e "\nAverage number of goals in all games from the winning teams rounded to two decimal places:"
echo "$($PSQL "SELECT ROUND(AVG(winner_goals), 2) FROM games")"

# Rata-rata jumlah gol dari kedua tim
echo -e "\nAverage number of goals in all games from both teams:"
echo "$($PSQL "SELECT AVG(winner_goals + opponent_goals) FROM games")"

# Jumlah gol terbanyak oleh satu tim dalam satu pertandingan
...
# Daftar unik tim pemenang di seluruh dataset
...
# Tim yang namanya diawali dengan 'Co'
